...
pub async fn overview_demons(connection: &mut PgConnection) -> Result<Vec<OverviewDemon>> {
    Ok(sqlx::query_as!(
        OverviewDemon,
        r#"SELECT demons.id, position, demons.hidden, demons.name as "name: String", CASE WHEN verifiers.link_banned THEN NULL ELSE video::TEXT END,
         players.name as "publisher: String", verifiers.name as "verifier: String" FROM demons INNER JOIN players ON demons.publisher = players.id
         INNER JOIN players AS verifiers ON demons.verifier = verifiers.id
         WHERE position IS NOT NULL AND hidden IS NOT true ORDER BY position"#
...
impl Page for DemonlistOverview {
    fn title(&self) -> String {
        "The Challenge List".to_string()
    }

    fn description(&self) -> String {
        "The hardest list of challenges, all concentrated in a simple listing on a website.".to_string()
    }
        ...
        vec![
            ...
            (PreEscaped(r#"
                ...
                <script type="application/ld+json">
                {
                    ...
                    "breadcrumb": {
                        ...
                        "itemListElement": [
                            ...
                            },
                            {
                                ...
                                "position": 2,
                                "item": {
                                    "@id": "https://pointercrate.com/challenges/",
                                    "name": "demonlist"
                                }
                            }
                        ]
                    ...
                    "url": "https://pointercrate.com/challenges/"
                }
                </script>
            "#))
            },
