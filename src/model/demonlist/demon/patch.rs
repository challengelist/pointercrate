...
impl Demon {
    ...
    pub async fn apply_patch(mut self, patch: PatchDemon, connection: &mut PgConnection) -> Result<Self> {
        ...
        if let Some(fps) = patch.fps {
            match fps {
                None => self.set_fps("Any".to_string(), connection).await?,
                Some(fps) => self.set_fps(fps, connection).await?,
            }
        }

        if let Some(hidden) = patch.hidden {
            self.set_hidden(hidden, connection).await?;
        }
    ...
    pub async fn set_fps(&mut self, fps: String, connection: &mut PgConnection) -> Result<()> {
        sqlx::query!("UPDATE demons SET fps = $1 WHERE id = $2", fps, self.base.id)
            .execute(connection)
            .await?;
    ...
    pub async fn set_hidden(&mut self, hidden: bool, connection: &mut PgConnection) -> Result<()> {
        sqlx::query!("UPDATE demons SET hidden = $1 WHERE id = $2", hidden, self.base.id)
            .execute(connection)
            .await?;
