...
export function initializeRecordSubmitter(csrf = null, submitApproved = false) {
  ...
  var raw = submissionForm.input("id_raw_footage");
  ...
  raw.addValidator(
    valueMissing,
    "Please attach the raw footage, as it's required for all records"
  );
  raw.addValidator(typeMismatch, "Please enter a valid URL");
...
export class StatsViewer extends FilteredPaginator {
  ...
  onReceive(response) {
    ...
    this._amountBeaten.textContent =
      (beaten.length - legacy - extended + playerData.verified.length - verifiedExtended - verifiedLegacy)
...
function formatDemonsInto(element, demons) {
  ...
  if (demons.length) {
    for (var demon of demons) {
      element.appendChild(
        formatDemon(demon, "/challenges/" + demon.position + "/")
      );
